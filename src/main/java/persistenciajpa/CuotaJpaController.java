...
public class CuotaJpaController implements Serializable {
    ...
    public void create(Cuota cuota) {
        ...
        try {
            ...
            if (pago != null) {
                pago = em.getReference(pago.getClass(), pago.getNroRecibo());
                cuota.setPago(pago);
            }
            em.persist(cuota);
            if (pago != null) {
                Cuota oldCuotaOfPago = pago.getCuota();
                if (oldCuotaOfPago != null) {
                    oldCuotaOfPago.setPago(null);
                    oldCuotaOfPago = em.merge(oldCuotaOfPago);
                }
                pago.setCuota(cuota);
                pago = em.merge(pago);
            }
    ...
    public void edit(Cuota cuota) throws NonexistentEntityException, Exception {
        ...
        try {
            ...
            if (pagoNew != null) {
                pagoNew = em.getReference(pagoNew.getClass(), pagoNew.getNroRecibo());
                cuota.setPago(pagoNew);
            }
            cuota = em.merge(cuota);
            if (pagoOld != null && !pagoOld.equals(pagoNew)) {
                pagoOld.setCuota(null);
                pagoOld = em.merge(pagoOld);
            }
            if (pagoNew != null && !pagoNew.equals(pagoOld)) {
                Cuota oldCuotaOfPago = pagoNew.getCuota();
                if (oldCuotaOfPago != null) {
                    oldCuotaOfPago.setPago(null);
                    oldCuotaOfPago = em.merge(oldCuotaOfPago);
                }
                pagoNew.setCuota(cuota);
